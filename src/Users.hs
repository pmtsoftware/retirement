module Users ( users, createUser, Form(..) ) where
...
createUser conn formData = case validateForm formData of
    Success data' -> catchViolation catcher $ Right <$> run conn data'
    Failure errs -> pure $ Left errs
    where
        catcher _ (UniqueViolation "email_unique") = pure . Left $ NE.singleton EmailNotUnique
        catcher e _ = throwIO e

        run :: Connection -> (Email, Password) -> IO UserId
        run conn (mail, pass) = do
